...
async function mintETH(argv: any, threadId: number) {
	const account = namedAccount("owner", threadId).connect(argv.provider);
	const startNonce = await account.getTransactionCount("pending");
	for (let index = 0; index < argv.times; index++) {
		const response = await account.sendTransaction({
			to: namedAddress(argv.to, threadId),
